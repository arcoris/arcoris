...
func TestReasonRegistryErrorsSupportIsAndAs(t *testing.T) {
	...
	duplicate := DuplicateReasonError{Reason: ReasonDenied}
	...
	if duplicateReason.Reason != ReasonDenied {
		t.Fatalf("duplicate reason = %q, want %q", duplicateReason.Reason, ReasonDenied)
	}
}
