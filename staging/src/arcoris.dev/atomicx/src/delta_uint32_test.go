...
// TestUint32CounterDeltaCannotDetectFullCycleWraps documents the two-sample
// ambiguity: equal samples mean zero visible progress, not proof that no full
// uint32 cycles occurred between samples.
func TestUint32CounterDeltaCannotDetectFullCycleWraps(t *testing.T) {
	t.Parallel()
...
func TestNewUint32CounterDeltaWrapped(t *testing.T) {
	...
	prev := testMaxUint32 - 2
	cur := uint32(4)

	delta := NewUint32CounterDelta(prev, cur)
