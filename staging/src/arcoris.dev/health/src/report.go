...
// in deterministic resolver/evaluator order. Report intentionally remains a
// plain value so callers can store, copy, render, or adapt it without owning
// evaluator state.
//
...
type Report struct {
	...
	// Checks contains normalized check Results in resolver/evaluator order.
	//
