...
func TestCatalogFixtureConstructorsReturnIndependentRegistries(t *testing.T) {
	t.Parallel()

	firstReasons := testReasonRegistry()
	secondReasons := testReasonRegistry()
	...
	if secondReasons.Contains("custom_reason") {
		t.Fatal("custom reason registered in one registry leaked into another")
	}

	firstKinds := testKindRegistry()
	secondKinds := testKindRegistry()
	...
	if secondKinds.Contains("custom_kind") {
		t.Fatal("custom kind registered in one registry leaked into another")
	}

	firstComponents := testComponentRegistry(firstKinds)
	secondComponents := testComponentRegistry(secondKinds)
	...
	if secondComponents.Contains("custom.component") {
		t.Fatal("custom component registered in one registry leaked into another")
	}
}
