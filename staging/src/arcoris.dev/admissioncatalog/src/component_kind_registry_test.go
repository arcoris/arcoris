...
func TestKindRegistryZeroValue(t *testing.T) {
	...
	if got, ok := registry.Lookup(testKindBulkhead); ok || got != (ComponentKindDescriptor{}) {
...
func TestKindRegistryListIsSortedCopy(t *testing.T) {
	...
	list := registry.List()
	gotOrder := []admission.ComponentKind{list[0].Kind, list[1].Kind, list[2].Kind}
	wantOrder := []admission.ComponentKind{"a_kind", "m_kind", "z_kind"}
...
func TestKindRegistryConcurrentAccess(t *testing.T) {
	...
	for i := 0; i < 32; i++ {
		...
		go func(i int) {
			...
			kind := admission.ComponentKind("custom_kind_" +
				string(rune('a'+i/26)) +
				string(rune('a'+i%26)))
...
func testKindDescriptor(kind admission.ComponentKind) ComponentKindDescriptor {
