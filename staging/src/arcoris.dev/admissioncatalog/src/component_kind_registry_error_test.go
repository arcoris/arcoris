...
func TestKindRegistryErrorsSupportIsAndAs(t *testing.T) {
	...
	duplicate := DuplicateComponentKindError{Kind: testKindBulkhead}
	...
	if duplicateKind.Kind != testKindBulkhead {
		t.Fatalf("duplicate kind = %q, want %q", duplicateKind.Kind, testKindBulkhead)
	}
}
