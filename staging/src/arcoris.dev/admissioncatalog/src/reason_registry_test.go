...
func TestReasonRegistryZeroValue(t *testing.T) {
	...
	if got, ok := registry.Lookup(admission.ReasonDenied); ok || got != (ReasonDescriptor{}) {
...
func TestReasonRegistryListIsSortedCopy(t *testing.T) {
	...
	list := registry.List()
	gotOrder := []admission.Reason{list[0].Reason, list[1].Reason, list[2].Reason}
	wantOrder := []admission.Reason{"a_reason", "m_reason", "z_reason"}
...
func TestReasonRegistryConcurrentAccess(t *testing.T) {
	...
	for i := 0; i < 32; i++ {
		...
		go func(i int) {
			...
			reason := admission.Reason("custom_reason_" +
				string(rune('a'+i/26)) +
				string(rune('a'+i%26)))
...
func testReasonDescriptor(reason admission.Reason) ReasonDescriptor {
