...
const nilReasonRegistryPanic = "admissioncatalog.ReasonRegistry: nil registry"

// ReasonRegistry is an owner-created catalog of known admission.Reason descriptors.
//
...
type ReasonRegistry struct {
	...
	byReason map[admission.Reason]ReasonDescriptor
}
...
func NewReasonRegistry(descriptors ...ReasonDescriptor) (*ReasonRegistry, error) {
	registry := &ReasonRegistry{
		byReason: make(map[admission.Reason]ReasonDescriptor, len(descriptors)),
	}
...
// not close the admission.Reason world: any syntactically valid reason may be
...
func (r *ReasonRegistry) Register(descriptor ReasonDescriptor) error {
	...
	if r.byReason == nil {
		r.byReason = make(map[admission.Reason]ReasonDescriptor, 1)
	}
...
func (r *ReasonRegistry) Lookup(reason admission.Reason) (ReasonDescriptor, bool) {
	r.requireNonNil()
	if !reason.IsValid() {
...
func (r *ReasonRegistry) Contains(reason admission.Reason) bool {
	_, ok := r.Lookup(reason)
	return ok
}

// List returns registered descriptors sorted by admission.Reason string.
//
