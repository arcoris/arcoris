...
const nilComponentRegistryPanic = "admissioncatalog.ComponentRegistry: nil registry"
...
type ComponentRegistry struct {
	...
	byID map[admission.ComponentID]ComponentDescriptor
}
...
func NewComponentRegistry(
	...
	registry := &ComponentRegistry{
		kinds: kinds,
		byID:  make(map[admission.ComponentID]ComponentDescriptor, len(descriptors)),
	}
...
func (r *ComponentRegistry) Register(descriptor ComponentDescriptor) error {
	...
	if r.byID == nil {
		r.byID = make(map[admission.ComponentID]ComponentDescriptor, 1)
	}
...
func (r *ComponentRegistry) Lookup(id admission.ComponentID) (ComponentDescriptor, bool) {
	r.requireNonNil()
	if !id.IsValid() {
...
func (r *ComponentRegistry) Contains(id admission.ComponentID) bool {
	_, ok := r.Lookup(id)
	return ok
}
