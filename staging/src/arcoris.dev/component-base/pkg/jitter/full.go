...
// Full returns a schedule that randomizes each available child delay in
...
//
// Full may produce values in:
//
//	[0, 100*time.Millisecond]
//	[0, 200*time.Millisecond]
//	[0, 400*time.Millisecond]
//
// Full is commonly useful with capped exponential schedules:
//
//	jitter.Full(
//	    delay.Cap(
//	        delay.Exponential(100*time.Millisecond, 2.0),
//	        2*time.Second,
//	    ),
//	)
//
// Full preserves child exhaustion. If the child sequence reports ok=false,
// the jittered sequence also reports ok=false. Full does not make a finite
...
//
// Full does not sleep, create timers, observe context cancellation,
...
//
// Full panics when schedule is nil or a random option is invalid.
func Full(sched delay.Schedule, opts ...RandomOption) delay.Schedule {
	return newJitterSchedule(sched, fullJitterTransform, opts...)
}
...
func fullJitterTransform(base time.Duration, r RandomGenerator) time.Duration {
	if base <= 0 {
		return 0
	}

	return randomDurationInclusive(r, base)
}
