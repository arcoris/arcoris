...
const (
	...
	errNilLimitSchedule = "backoff: nil limit schedule"
	...
	errNegativeLimitMaxDelays = "backoff: negative limit maximum delays"
	...
	errLimitScheduleReturnedNilSequence = "backoff: limit schedule returned nil Sequence"
	...
	// at the backoff boundary instead of being silently hidden.
	errLimitScheduleReturnedNegativeDelay = "backoff: limit schedule returned negative delay"
)
...
//   - bounding retry delay budgets at the backoff layer;
...
//
// Limit panics when schedule is nil or maxDelays is negative.
func Limit(schedule Schedule, maxDelays int) Schedule {
	requireSchedule(schedule, errNilLimitSchedule)
	requireNonNegativeCount(maxDelays, errNegativeLimitMaxDelays)
...
func (s *limitSequence) Next() (time.Duration, bool) {
	...
	requireNonNegativeSequenceDelay(delay, ok, errLimitScheduleReturnedNegativeDelay)
