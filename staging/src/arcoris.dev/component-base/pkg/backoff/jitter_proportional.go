...
// ProportionalJitter returns a schedule that randomizes each available child
...
//
// Proportional jitter is useful when FullJitter is too aggressive but positive
...
// For example:
//
//	backoff.ProportionalJitter(backoff.Fixed(time.Second), 0.2)
//
// may produce values in:
//
//	[800*time.Millisecond, 1200*time.Millisecond]
//
// ProportionalJitter preserves child exhaustion. If the child sequence reports
...
//
// ProportionalJitter does not sleep, create timers, observe context
...
// domain decisions.
//
// ProportionalJitter panics when schedule is nil, ratio is outside [0, 1], NaN,
// or infinite, or a random option is invalid.
func ProportionalJitter(schedule Schedule, ratio float64, opts ...RandomOption) Schedule {
	requireJitterRatio(ratio)

	return newJitterSchedule(schedule, proportionalJitterTransform(ratio), opts...)
}
...
// non-negative.
func proportionalJitterTransform(ratio float64) jitterTransform {
	return func(baseDelay time.Duration, random RandomGenerator) time.Duration {
		if baseDelay <= 0 || ratio == 0 {
			return baseDelay
		}

		delta := saturatingDurationMulFloat(baseDelay, ratio)

		lower := saturatingDurationSub(baseDelay, delta)
		upper := saturatingDurationAdd(baseDelay, delta)
