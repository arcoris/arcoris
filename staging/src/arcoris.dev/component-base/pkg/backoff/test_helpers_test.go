...
//
// Backoff constructor failures are programming errors and use stable
...
// mustNext asserts that sequence returns want with ok=true.
func mustNext(t *testing.T, sequence Sequence, want time.Duration) {
	t.Helper()
...
// mustNextInRange asserts that sequence returns a delay in [min, max].
func mustNextInRange(t *testing.T, sequence Sequence, min, max time.Duration) time.Duration {
	t.Helper()

	got, ok := sequence.Next()
	if !ok {
		t.Fatalf("Next() exhausted, want delay in [%s, %s]", min, max)
	}
	if got < min || got > max {
		t.Fatalf("Next() delay = %s, want in [%s, %s]", got, min, max)
	}
	return got
}

// mustExhausted asserts that sequence reports finite exhaustion.
func mustExhausted(t *testing.T, sequence Sequence) {
	t.Helper()
