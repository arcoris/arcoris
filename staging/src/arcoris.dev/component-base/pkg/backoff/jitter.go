...
const (
	...
	errNilJitterSchedule = "backoff: nil jitter schedule"
	...
	errNilJitterTransform = "backoff: nil jitter transform"
	...
	errInvalidJitterFactor = "backoff: invalid jitter factor"
	...
	errInvalidJitterRatio = "backoff: invalid jitter ratio"
	...
	errJitterScheduleReturnedNilSequence = "backoff: jitter schedule returned nil Sequence"
	...
	errJitterScheduleReturnedNegativeDelay = "backoff: jitter schedule returned negative delay"
	...
	errJitterTransformReturnedNegativeDelay = "backoff: jitter transform returned negative delay"
)
...
// decisions.
type jitterTransform func(baseDelay time.Duration, random RandomGenerator) time.Duration
...
func newJitterSchedule(schedule Schedule, transform jitterTransform, opts ...RandomOption) Schedule {
	config := randomOptionsOf(opts...)
	return newJitterScheduleWithSource(schedule, transform, config.source)
}
...
func newJitterScheduleWithSource(
	...
	requireSchedule(schedule, errNilJitterSchedule)
	if transform == nil {
...
// sequence.
func (s jitterSchedule) NewSequence() Sequence {
...
func (s *jitterSequence) Next() (time.Duration, bool) {
	baseDelay, ok := s.child.Next()
	if !ok {
		return 0, false
	}
	requireNonNegativeSequenceDelay(baseDelay, ok, errJitterScheduleReturnedNegativeDelay)

	delay := s.transform(baseDelay, s.random)
	if delay < 0 {
