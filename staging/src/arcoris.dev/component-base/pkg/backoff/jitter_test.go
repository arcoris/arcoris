...
func TestNewJitterScheduleRejectsInvalidInput(t *testing.T) {
	...
	mustPanicWith(t, errNilJitterTransform, func() {
		newJitterSchedule(Fixed(time.Second), nil)
	})
	mustPanicWith(t, errNilRandomOption, func() {
		newJitterSchedule(Fixed(time.Second), fullJitterTransform, nil)
	})
	mustPanicWith(t, errNilRandomSource, func() {
		newJitterScheduleWithSource(Fixed(time.Second), fullJitterTransform, nil)
	})
}

func TestJitterPreservesChildExhaustion(t *testing.T) {
	sequence := newJitterSchedule(Delays(), fullJitterTransform, WithRandom(fixedRandom(0))).NewSequence()
...
func TestJitterRejectsNegativeChildDelay(t *testing.T) {
	sequence := newJitterSchedule(ScheduleFunc(func() Sequence { return negativeDelaySequence{} }), fullJitterTransform).NewSequence()
...
func TestJitterRejectsNegativeTransformOutput(t *testing.T) {
	sequence := newJitterSchedule(Fixed(time.Second), func(time.Duration, RandomGenerator) time.Duration {
...
func TestJitterRequestsRandomGeneratorPerSequence(t *testing.T) {
	...
	schedule := newJitterScheduleWithSource(Fixed(time.Second), fullJitterTransform, source)
