...
const (
	...
	// clock, retry, polling, reconnect, or cooldown interpretation. The
	...
	errNegativeLinearInitialDelay = "backoff: negative linear initial delay"

	// errNegativeLinearStep is the stable diagnostic text used when Linear
	// receives a negative step.
	//
	// Linear backoff is monotonic by construction: each produced delay is the
	...
	// durations. The constructor panics immediately instead of silently accepting
	// a schedule that violates the linear backoff contract.
	errNegativeLinearStep = "backoff: negative linear step"
)
...
//
// Linear does not provide as much overload relief as exponential backoff. For
// remote dependencies under sustained failure, callers should usually prefer a
// capped exponential schedule with jitter once that schedule is available.
//
...
func (s *linearSequence) Next() (time.Duration, bool) {
	delay := linearDelay(s.initial, s.step, s.next)
