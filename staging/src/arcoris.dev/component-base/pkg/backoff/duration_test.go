...
func TestDurationHelpersSaturateInsteadOfWrapping(t *testing.T) {
	tests := []struct {
		...
		{name: "max", got: maxDurationValue(time.Second, time.Millisecond), want: time.Second},
	}
...
func TestDurationSignPredicates(t *testing.T) {
	...
	if !isPositiveDuration(time.Nanosecond) {
		t.Fatal("positive duration was not reported positive")
	}
}
