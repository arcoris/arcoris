...
const (
	...
	// when DecorrelatedJitter receives a non-positive initial delay.
	//
	...
	errNonPositiveDecorrelatedInitialDelay = "backoff: non-positive decorrelated jitter initial delay"

	// errDecorrelatedMaxDelayBeforeInitialDelay is the stable diagnostic text
	// used when DecorrelatedJitter receives a maximum delay smaller than the
	// initial delay.
	//
	...
	errDecorrelatedMaxDelayBeforeInitialDelay = "backoff: decorrelated jitter maximum delay before initial delay"

	// errInvalidDecorrelatedMultiplier is the stable diagnostic text used when
	// DecorrelatedJitter receives an invalid multiplier.
	//
	...
	errInvalidDecorrelatedMultiplier = "backoff: invalid decorrelated jitter multiplier"

	// errNilDecorrelatedJitterSource is the stable diagnostic text used when the
	// package-local decorrelated jitter constructor receives a nil RandomSource.
	errNilDecorrelatedJitterSource = "backoff: nil decorrelated jitter source"
)

// DecorrelatedJitter returns an infinite decorrelated-jitter schedule.
//
// Decorrelated jitter is a stateful randomized backoff algorithm. Each produced
...
//
// DecorrelatedJitter is useful for advanced retry and reconnect paths where
...
//
// DecorrelatedJitter is not a wrapper around another Schedule. It owns its own
// sequence state because its algorithm depends on the previously returned delay.
//
// DecorrelatedJitter does not sleep, create timers, observe context
...
//
// DecorrelatedJitter panics when initial is not positive, maxDelay is smaller
...
func DecorrelatedJitter(
	...
	return decorrelatedJitterWithSource(initial, maxDelay, multiplier, config.source)
}
...
func decorrelatedJitterWithSource(
	...
	requireRandomSource(source, errNilDecorrelatedJitterSource)
...
// decorrelatedJitterSchedule is the reusable recipe behind DecorrelatedJitter.
//
...
func (s decorrelatedJitterSchedule) NewSequence() Sequence {
