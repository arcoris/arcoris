...
// provides owner-controlled waiting and cleanup. Subscription values must be
// created with Subscribe or SubscribeWithOptions and must not be copied after
// first use.
...
//
// An empty sigs list means Shutdown. Subscribe panics when any signal is nil.
...
//
// An empty sigs list means Shutdown. Duplicate signals are removed while
// preserving first occurrence order. The returned Subscription owns its signal
// registration until Stop is called.
func SubscribeWithOptions(sigs []os.Signal, opts ...SubscribeOption) *Subscription {
	config := newSubscribeConfig(opts...)
	registered := signalSetOrShutdown(sigs)
...
//
// Stop is idempotent. It does not close C. It closes Done so goroutines waiting
// through Wait can leave with ErrStopped.
