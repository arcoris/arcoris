...
var (
	// ErrTaskFailed classifies errors produced by failed taskgroup tasks.
	//
	// TaskError values match ErrTaskFailed with errors.Is while preserving the
	// original task error through Unwrap.
	ErrTaskFailed = errors.New("taskgroup task failed")
)
...
func (e TaskError) Error() string {
	if e.Name == "" {
		return fmt.Sprintf("taskgroup task failed: %v", e.Err)
	}

	return fmt.Sprintf("taskgroup task %q failed: %v", e.Name, e.Err)
}
