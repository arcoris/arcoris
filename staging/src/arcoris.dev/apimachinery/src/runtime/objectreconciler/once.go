...
//
// ReconcileOnce validates request, reads exactly one snapshot from source,
// converts it to this package's Snapshot type, and calls reconciler at most
// once. It preserves source and user errors as-is and does not recover panics
// from user code. Source read errors, including objectcache.ErrNotReady, are
// returned without wrapping through Result.Err.
//
...
func ReconcileOnce(
	...
	return reconciler.Reconcile(ctx, request, snapshotFromSource(sourceSnapshot))
}
