...
func TestCompareSameStringIsEmpty(t *testing.T) {
	got, err := Compare(value.StringValue("a"), value.StringValue("a"), types.String().Descriptor(), Options{})
...
func TestCompareFloatSameIsEmpty(t *testing.T) {
	got, err := Compare(mustFloat(t, 1.25), mustFloat(t, 1.25), types.Float64().Descriptor(), Options{})
	requireNoError(t, err)
	requireResult(t, got, nil, nil, nil)
}

func TestCompareFloatNegativeZeroEqualsZero(t *testing.T) {
	got, err := Compare(mustFloat(t, math.Copysign(0, -1)), mustFloat(t, 0), types.Float64().Descriptor(), Options{})
	requireNoError(t, err)
	requireResult(t, got, nil, nil, nil)
}

func TestCompareDecimalEqualDifferentScaleIsEmpty(t *testing.T) {
	got, err := Compare(mustDecimal(t, "1.0"), mustDecimal(t, "1.00"), types.Decimal().Descriptor(), Options{})
	requireNoError(t, err)
	requireResult(t, got, nil, nil, nil)
}
...
func TestCompareNullToNullIsEmpty(t *testing.T) {
	got, err := Compare(value.NullValue(), value.NullValue(), types.String().Nullable().Descriptor(), Options{})
	requireNoError(t, err)
	requireResult(t, got, nil, nil, nil)
}
