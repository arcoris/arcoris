...
func TestUint16FieldWrapper(t *testing.T) {
	field := Field("value").Uint16().Required().Nullable().Range(0, 2).Enum(0, 1, 2).Description("value").Field()
...
func TestUint16FieldExprMarker(t *testing.T) {
	Field("value").Uint16().fieldExpr()
}

func TestUint16FieldOptionalPath(t *testing.T) {
	field := Field("value").Uint16().Optional().Min(1).Max(2).Field()
