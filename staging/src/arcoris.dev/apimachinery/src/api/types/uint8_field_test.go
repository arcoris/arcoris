...
func TestUint8FieldWrapper(t *testing.T) {
	field := Field("value").Uint8().Required().Nullable().Range(0, 2).Enum(0, 1, 2).Description("value").Field()
...
func TestUint8FieldExprMarker(t *testing.T) {
	Field("value").Uint8().fieldExpr()
}

func TestUint8FieldOptionalPath(t *testing.T) {
	field := Field("value").Uint8().Optional().Min(1).Max(2).Field()
