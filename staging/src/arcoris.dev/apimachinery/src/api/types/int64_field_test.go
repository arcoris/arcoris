...
func TestInt64FieldWrapper(t *testing.T) {
	field := Field("value").Int64().Required().Nullable().Range(-1, 1).Enum(-1, 0, 1).Description("value").Field()
...
func TestInt64FieldExprMarker(t *testing.T) {
	Field("value").Int64().fieldExpr()
}

func TestInt64FieldOptionalPath(t *testing.T) {
	field := Field("value").Int64().Optional().Min(-1).Max(1).Field()
