...
func (f StringField) Required() StringField { f.field = f.field.withRequired(); return f }

// Optional marks the field key as optional.
func (f StringField) Optional() StringField { f.field = f.field.withOptional(); return f }
...
func (f StringField) Nullable() StringField { f.typ = f.typ.Nullable(); return f }

// MinLen sets the inclusive minimum string length.
func (f StringField) MinLen(n int) StringField { f.typ = f.typ.MinLen(n); return f }

// MaxLen sets the inclusive maximum string length.
func (f StringField) MaxLen(n int) StringField { f.typ = f.typ.MaxLen(n); return f }

// Pattern stores a textual regular expression for the string field.
func (f StringField) Pattern(pattern string) StringField { f.typ = f.typ.Pattern(pattern); return f }

// Enum stores accepted string literals for the field.
func (f StringField) Enum(values ...string) StringField { f.typ = f.typ.Enum(values...); return f }

// Field returns a detached finalized field descriptor.
func (f StringField) Field() FieldDescriptor { return f.field.fieldWithType(f.typ.Type()) }
