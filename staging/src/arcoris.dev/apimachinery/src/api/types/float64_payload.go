...
// float64Payload stores DescriptorFloat64 constraints in their exact native width.
type float64Payload struct {
	// min stores the inclusive finite lower bound for DescriptorFloat64.
	min limit[float64]
	// max stores the inclusive finite upper bound for DescriptorFloat64.
	max limit[float64]
...
// emptyFloat64Payload reports whether p has no configured DescriptorFloat64 state.
