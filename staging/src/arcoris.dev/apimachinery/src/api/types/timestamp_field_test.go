...
func TestTimestampFieldWrapper(t *testing.T) {
	field := Field("value").Timestamp().Required().Nullable().Description("value").Field()
...
func TestTimestampFieldExprMarker(t *testing.T) {
	Field("value").Timestamp().fieldExpr()
}

func TestTimestampFieldOptionalPath(t *testing.T) {
	field := Field("value").Timestamp().Optional().Field()
