...
// mergeMap recursively merges dynamic string-keyed map entries.
...
func (m *merger) mergeMapView(
	...
	members, err := m.mergeBaseMapMembers(
		path,
		baseMembers,
		overlayLookup,
		valueDescriptor,
	...
	members, err = m.appendOverlayMapMembers(
		path,
		members,
		baseLookup,
		overlayMembers,
