...
// terminal error. Implementations must not emit events after terminal
// restart/continuity loss. Close should be idempotent when practical.
type Stream interface {
	// Next returns the next validated event or a terminal/context error.
	Next(context.Context) (Event, error)
	// Close releases stream resources. Implementations should make Close
