...
func TestNameScalarEncoding(t *testing.T) {
	text, err := Name("worker").MarshalText()
	...
	var name Name
	requireNoError(t, name.UnmarshalText([]byte("worker")))
	if name != "worker" {
		t.Fatalf("UnmarshalText() = %q", name)
	}

	data, err := json.Marshal(Name("worker"))
	requireNoError(t, err)
