...
func TestValidateOpaqueToken(t *testing.T) {
	if err := ValidateOpaqueToken("token", "", true); err != nil {
		t.Fatalf("ValidateOpaqueToken(empty allowed) error = %v", err)
	}
	if err := ValidateOpaqueToken("token", "opaque-1", false); err != nil {
		t.Fatalf("ValidateOpaqueToken() error = %v", err)
	}
	if err := ValidateOpaqueToken("token", "", false); err == nil {
		t.Fatal("ValidateOpaqueToken(empty required) error = nil")
	}
	if err := ValidateOpaqueToken("token", "bad token", true); err == nil {
