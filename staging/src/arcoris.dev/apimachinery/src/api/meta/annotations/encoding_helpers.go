...
// marshalText validates an annotation scalar before exposing it as text.
func marshalText(value string, validate func() error) ([]byte, error) {
	return metaencoding.MarshalText(value, validate)
}

// marshalJSONString validates an annotation scalar before encoding it as one JSON string.
