...
// Equal returns a schedule that randomizes each available child delay in
...
//
// Equal is useful when Full is too aggressive because it may return
// very small delays. Equal still desynchronizes owners, but it preserves a
...
//
// Equal may produce values in:
//
//	[50*time.Millisecond, 100*time.Millisecond]
//	[100*time.Millisecond, 200*time.Millisecond]
//	[200*time.Millisecond, 400*time.Millisecond]
//
// Equal preserves child exhaustion. If the child sequence reports
// ok=false, the jittered sequence also reports ok=false. Equal does not
...
//
// Equal does not sleep, create timers, observe context cancellation,
...
//
// Equal panics when schedule is nil or a random option is invalid.
func Equal(sched delay.Schedule, opts ...RandomOption) delay.Schedule {
	return newJitterSchedule(sched, equalJitterTransform, opts...)
}
...
func equalJitterTransform(base time.Duration, r RandomGenerator) time.Duration {
	if base <= 0 {
		return 0
	}

	lower := base / 2
	span := saturatingDurationSub(base, lower)

	return saturatingDurationAdd(lower, randomOffsetInclusive(r, span))
}
