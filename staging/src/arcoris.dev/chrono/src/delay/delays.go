...
const (
	// errNegativeExplicitDelay is the stable diagnostic text used when Delays
	...
	errNegativeExplicitDelay = "delay: negative explicit delay"
)
...
func Delays(ds ...time.Duration) Schedule {
	copied := make([]time.Duration, len(ds))
	copy(copied, ds)

	for _, d := range copied {
		requireNonNegativeDuration(d, errNegativeExplicitDelay)
	}
