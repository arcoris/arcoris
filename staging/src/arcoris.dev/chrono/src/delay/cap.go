...
const (
	...
	errNilCapSchedule = "delay: nil cap schedule"
	...
	errNegativeCapMaxDelay = "delay: negative cap maximum delay"
	...
	errCapScheduleReturnedNilSequence = "delay: cap schedule returned nil Sequence"
	...
	// immediately so the child contract violation is visible at the delay
	// boundary instead of being silently hidden.
	errCapScheduleReturnedNegativeDelay = "delay: cap schedule returned negative delay"
)
...
//
// Cap panics when sched is nil or m is negative.
func Cap(sched Schedule, m time.Duration) Schedule {
	requireSchedule(sched, errNilCapSchedule)
	requireNonNegativeDuration(m, errNegativeCapMaxDelay)
...
func (s capSequence) Next() (time.Duration, bool) {
	...
	requireNonNegativeSequenceDelay(d, ok, errCapScheduleReturnedNegativeDelay)

	return capDuration(d, s.maxDelay), true
}
