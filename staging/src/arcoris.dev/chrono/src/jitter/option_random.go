...
const (
	...
	// is reported at the jitter boundary.
	errNilRandomOption = "jitter: nil random option"
	...
	errNilRandomConfig = "jitter: nil random config"
)

// RandomOption configures non-cryptographic randomness for uniform schedules and
// jitter schedules.
//
...
func randomOptionsOf(opts ...RandomOption) randomConfig {
	...
	for _, opt := range opts {
		requireRandomOption(opt, errNilRandomOption)
		opt(&cfg)
	}
	requireRandomConfig(&cfg, errNilRandomConfig)
	requireRandomSource(cfg.source, errNilRandomSource)
	return cfg
}
...
//	})
//	schedule := jitter.Full(delay.Fixed(time.Second), WithRandomSource(source))
//
// WithRandomSource panics when src is nil.
func WithRandomSource(src RandomSource) RandomOption {
...
// Example:
//
//	schedule := jitter.Uniform(0, time.Second, WithRandom(myTestRandom))
//
// WithRandom panics when r is nil.
func WithRandom(r RandomGenerator) RandomOption {
...
// Example:
//
//	schedule := jitter.Uniform(0, time.Second, WithRandomFunc(func() int64 {
//		return 0
//	}))
//
